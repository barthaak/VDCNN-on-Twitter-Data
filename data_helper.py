...
class data_helper():
    ...
    def load_dataset(self, dataset_path,training=True):
        # Read Classes Info
        try:
        ...
        if training:
            ...
            if 'twitter_hate_csv' in dataset_path:
                test_data, test_label = train_data[3250:4000], train_label[3250:4000] 
                train_data, train_label = train_data[:3250], train_label[:3250]
            elif 'twitter_semeval_csv' in dataset_path:
                test_data, test_label = train_data[3250:], train_label[3250:] 
                train_data, train_label = train_data[:3250], train_label[:3250]
            elif 'twitter_sentiment_csv' in dataset_path:
                test_data, test_label = train_data[8000:], train_label[8000:] 
                train_data, train_label = train_data[:8000], train_label[:8000]
            else:
            ...
            if 'twitter_hate_csv' in dataset_path:
                test_data, test_label = self.load_csv_file(dataset_path+'train.csv', num_classes)
                test_data, test_label = test_data[4000:], test_label[4000:]
            elif ('twitter_semeval_csv' in dataset_path) or ('twitter_sentiment_csv' in dataset_path):
